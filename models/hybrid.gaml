...
global { 
	...
	int population <- 500 	min:1 max:1000;
	...
	*/
	...
	init {
     ...
     //determine the meso block where the cell belongs
		ask cell{
			block <- one_of(meso_block where(each.group_no = group_no));			
		}
...
species people {
	...
	action update_waypoints(cell current, cell target){
		...
		int angle <- 0;
			...
			exit_angle <- abs(angle_between({current.getCenter().x,-current.getCenter().y}, { current.getCenter().x +1, -current.getCenter().y  },  {target.getCenter().x,-target.getCenter().y} ));
			...
			entry_angle <- abs(angle_between({target.getCenter().x,-target.getCenter().y}, { target.getCenter().x +1, -target.getCenter().y  },  {current.getCenter().x,-current.getCenter().y} ));
...
experiment main type: gui {	
	...
	parameter "Population" var: population <- 500;
	output {
		display default{
...
experiment Tabu_Search type: batch repeat: 5 keep_seed: true until:empty(people) 
{
	...
	parameter "Population" var: population <- 1000;
	...
	method tabu iter_max: 500 tabu_list_size: 5 minimize: egress_time;
	reflex end_of_runs
	{

			string line <- "" + Ia + "," + Ib + "," + Sa + "," + Sb + "," + Na + "," + Nb + "," + Navigation_bias + "," + egress_time + "," + ave_travel_time + "," + ave_walking_distance + "," + ave_walking_speed;
	
		    save (line) to: "../includes/paper/" + folder + "/parameters/egress_"+ population type: "csv" header: false rewrite: false;
			write line;
